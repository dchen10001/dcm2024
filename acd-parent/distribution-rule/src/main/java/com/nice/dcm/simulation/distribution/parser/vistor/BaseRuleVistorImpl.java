...
public abstract class BaseRuleVistorImpl implements RuleVistor {
    ...
    private BinaryConditionImpl toCondition(BinaryOperatorContext binaryOperator, List<TerminalNode> numbers) {
        if (binaryOperator == null) {
            return null;
        }
        BinaryOperatorNodeImpl binaryOperatorNode = visitBinaryOperator(binaryOperator);
        int level = this.toNumber(numbers.get(0));
        return new BinaryConditionImpl(binaryOperatorNode.getOperator(), level);
    }

    private SqlConditionImpl toCondition(SqlOperatorContext sqlOperator, List<TerminalNode> numbers) {
        if (sqlOperator == null) {
            return null;
        }
        ...
        return new SqlConditionImpl(sqlOperatorNode.getOperator(), lowerLevel, upperLevel);
    }    
}
