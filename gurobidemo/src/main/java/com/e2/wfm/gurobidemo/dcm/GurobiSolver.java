...
import com.e2.wfm.gurobidemo.dcm.util.ClockWatch;
...
public class GurobiSolver {
	...
	private ClockWatch clockWatch = new ClockWatch();
	...
	public void initModel() throws GRBException {
		...
		addMaxVariableas();
		clockWatch.stop("Model initialization");
	}

	public int solve() throws GRBException {
		model.write("model123.lp");
		model.write("model.rlp");
		model.write("model.mps");
		model.write("model.rew");
		clockWatch.start();
		model.optimize();
		clockWatch.stop("Model optimization");
		return model.get(GRB.IntAttr.Status);
	}

	public int solveMip() throws GRBException {
		clockWatch.start();
		EmployeeConstraint[] employeeConstraints = this.register.getEmployeeConstraints();
		for (EmployeeConstraint employeeConstraint : employeeConstraints) {
            Collection<ScheduleCandidate> scheduleVars = employeeConstraint.getScheduleVars().values();
            for (ScheduleCandidate scheduleVar : scheduleVars) {
            	GRBVar schedule = scheduleVar.getScheduleVar();
            	schedule.set(GRB.CharAttr.VType, GRB.BINARY);
            }
		}
		
		clockWatch.stop("Model MIP update");
		clockWatch.start();
		int ret = this.solve();
		clockWatch.stop("Model MIP optimization");
		return ret;
	}
	...
	public DCMModelOutput getValues() throws GRBException {
		...
		clockWatch.stop("Model get dual value");
	...
	private EmployeeValue getEmployeeValue(EmployeeConstraint employeeConstraint) throws GRBException {
		...
		return new EmployeeValue(employeeConstraint.getId(), employeeConstraint.getGroupId(),
				employeeConstraint.getSchedulingUnitId(), scheduleValues, dualValue);
	}
	...
	protected SchedulingUnitValue getSchedulingUnitValue(SchedulingUnitConstraint schedulingUnitConstraint)
			throws GRBException {
		...
		return new SchedulingUnitValue(schedulingUnitConstraint.getId(), dualValues, lowerSlackValues,
				upperSlackValues);
	}
	...
	private SchedulingUnitConstraint addSeatLimitConstraint(SchedulingUnit schedulingUnit) throws GRBException {
		...
		SchedulingUnitConstraint constraint = new SchedulingUnitConstraint(schedulingUnitId, constrs, lowerSlacks,
				upperSlacks);
		...
		for (int i = 0; i < interval; i++) {
			...
			lowerSlacks[i] = model.addVar(0.0, GRB.INFINITY, this.dcmModel.getLowerCoeff(), GRB.CONTINUOUS,
					"ls_l" + schedulingUnitId + "_i" + i);
			expr.addTerm(1.0, lowerSlacks[i]);
			upperSlacks[i] = model.addVar(0.0, GRB.INFINITY, this.dcmModel.getUpperCoeff(), GRB.CONTINUOUS,
					"us_l" + schedulingUnitId + "_i" + i);
			expr.addTerm(-1.0, upperSlacks[i]);
			constrs[i] = model.addRange(expr, minSeatLimits[i], maxSeatLimits[i], "sl_l" + schedulingUnitId + "_i" + i);
		}
		return constraint;
	}
	...
	private ContactTypeConstraint addContactTypeConstraints(ContactType contactType) throws GRBException {
		...
		for (int i = 0; i < interval; i++) {
			GRBLinExpr expr = new GRBLinExpr();
			unders[i] = model.addVar(0.0, GRB.INFINITY, this.dcmModel.getUnderCoeff(), GRB.CONTINUOUS,
					"under_t" + contactTypeId + "_i" + i);
			expr.addTerm(1.0, unders[i]);
			overs[i] = model.addVar(0.0, GRB.INFINITY, this.dcmModel.getOverCoeff(), GRB.CONTINUOUS,
					"over_t" + contactTypeId + "_i" + i);
	...
	private MaxContactTypeConstraint addMaxVariablea(ContactType contactType) throws GRBException {
		...
		GRBVar ctMaxUnder = model.addVar(0.0, GRB.INFINITY, this.dcmModel.getMaxUnderCoeff(), GRB.CONTINUOUS,
				"ctMaxUnder_t" + contactTypeId);
		...
		for (int i = 0; i < underVars.length; i++) {
			...
			ctMaxUnderConstrs[i] = model.addConstr(linExpr, GRB.GREATER_EQUAL, 0d,
					"ctMaxUnderConstr_t" + contactTypeId + "_i" + i);
		}

		GRBVar ctMaxOver = model.addVar(0.0, GRB.INFINITY, this.dcmModel.getMaxOverCoeff(), GRB.CONTINUOUS,
				"ctMaxOver_t" + contactTypeId);
		...
		for (int i = 0; i < overVars.length; i++) {
			...
			ctMaxOverConstrs[i] = model.addConstr(linExpr, GRB.GREATER_EQUAL, 0d,
					"ctMaxOverConstr_t" + contactTypeId + "_i" + i);
		}
	...
	public void nextIteration(List<EmployeeSchedule> schedules, int iteration) throws GRBException {
		...
		this.clockWatch.start();
		updateScheduleVariables();
		this.clockWatch.stop("Update model");
	}

	private void addEmployeeSchedule(EmployeeConstraint constraint, EmployeeSchedule schedule, int iteration)
			throws GRBException {
	...
	private EmployeeConstraint addInitEmployeeConstraint(Employee employee) throws GRBException {
		...
		// ∑_jSaj=1
		GRBVar schedule = model.addVar(0.0, GRB.INFINITY, 0.0d, GRB.CONTINUOUS, "s_a" + empId + "_j" + 0);

		GRBLinExpr expr = new GRBLinExpr();
		expr.addTerm(1.0, schedule);
		GRBConstr constr = model.addConstr(expr, GRB.EQUAL, 1.0, "OneCandidatePer_a" + empId);

		return new EmployeeConstraint(empId, employee.getSkillGroupId(), employee.getSchedulingUnitId(), constr,
				schedule, employee.getSchedules());
	}
	...
	private void updateEmployeeSchedule(EmployeeConstraint employeeConstraint) throws GRBException {
		...
		SchedulingUnitConstraint schedulingUnitConstraint = this.register
				.getSchedulingUnitConstraint(employeeConstraint.getSchedulingUnitId());
	...
	protected double getDualValue(GRBConstr constr) {
		if (constr == null) {
			return 0.0;
		}
		try {
